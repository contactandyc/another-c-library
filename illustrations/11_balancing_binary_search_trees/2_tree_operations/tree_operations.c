...
void post_good_tree(ac_pool_t *pool, node_t *root) {
  tree_log_t *n = (tree_log_t *)ac_pool_alloc(pool, sizeof(tree_log_t));
...
void do_test_red_black(ac_pool_t *pool, node_t *root, char *p) {
  tree_log_t *n = (tree_log_t *)ac_pool_alloc(pool, sizeof(tree_log_t));
  n->tree = tree_copy(pool, root);
  n->op = p;
  n->next = NULL;
...
void do_paint_red(ac_pool_t *pool, node_t *root, char *p, bool replay) {
  char key;
  p = get_key(p, &key);
  if(!key) {
    ...
    if(n) {
      ...
      if(!replay) {
        ...
        do_test_red_black(pool, root, ac_pool_strdupf(pool, "Red %c", key));
      }
    }
    else
      printf( "%c not found\n", key );
...
void do_paint_black(ac_pool_t *pool, node_t *root, char *p, bool replay) {
  char key;
  p = get_key(p, &key);
  if(!key) {
    printf( "ERROR: key not specified\n\n");
    ...
    if(n) {
      ...
      if(!replay) {
        node_print(pool, root);
        do_test_red_black(pool, root, ac_pool_strdupf(pool, "black %c", key));
      }
    }
    else
      printf( "%c not found\n", key );
...
void do_recolor(ac_pool_t *pool, node_t *root, char *p, bool replay) {
  char key;
  p = get_key(p, &key);
  if(!key) {
    printf( "ERROR: key not specified\n\n");
    ...
    if(n) {
      if(n->color == BLACK && n->left && n->left->color == RED && n->right && n->right->color == RED) {
        ...
        if(!replay) {
          node_print(pool, root);
          do_test_red_black(pool, root, ac_pool_strdupf(pool, "color %c", key));
        }
      }
      else {
...
void do_insert(ac_pool_t *pool, node_t **root, char *p, bool replay) {
  char key;
  p = get_key(p, &key);
  if(!key) {
    printf( "ERROR: key not specified\n\n");
    ...
    if(!n) {
      ...
      if(!replay) {
        node_print(pool, *root);
        do_test_red_black(pool, *root, ac_pool_strdupf(pool, "insert %c", key));
      }
    }
    else
...
void do_erase(ac_pool_t *pool, node_t **root, char *p, bool replay) {
  char key;
  p = get_key(p, &key);
  if(!key) {
    printf( "ERROR: key not specified\n\n");
    ...
    if(n) {
      ...
      if(!replay) {
        node_print(pool, *root);
        do_test_red_black(pool, *root, ac_pool_strdupf(pool, "erase %c", key));
      }
    }
    else
...
void do_right_rotate(ac_pool_t *pool, node_t **root, char *p, bool replay) {
  char key;
  p = get_key(p, &key);
  if(!key) {
    printf( "ERROR: key not specified\n\n");
    ...
    if(n) {
      if(n->left) {
        ...
        if(!replay) {
          node_print(pool, *root);
          do_test_red_black(pool, *root, ac_pool_strdupf(pool, "right_rotate %c", key));
        }
      }
      else
...
void do_left_rotate(ac_pool_t *pool, node_t **root, char *p, bool replay) {
  char key;
  p = get_key(p, &key);
  if(!key) {
    printf( "ERROR: key not specified\n\n");
    ...
    if(n) {
      if(n->right) {
        ...
        if(!replay) {
          node_print(pool, *root);
          do_test_red_black(pool, *root, ac_pool_strdupf(pool, "left_rotate %c", key));
        }
      }
      else
...
void tree_operations(ac_pool_t *pool, char *args) {
