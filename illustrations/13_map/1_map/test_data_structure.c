...
char * print_node(acpool_t *pool, acmap_node_t *n) {
  node_t *node = (node_t *)n;
  return acpool_strdupf(pool, "%c", node->key );
}


acmap_insert_m(node_insert, node_t, compare_node);
acmap_find_m(node_find, char, node_t, compare_node_for_find);
...
*/
acmap_node_t *fill_data_structure(acpool_t *pool, const char *arg) {
  acmap_node_t *root = NULL;
  const char *s = arg;
  while (*s != 0) {
    if (!node_find(*s, root)) {
      node_t *n = (node_t *)acpool_alloc(pool, sizeof(*n));
      n->key = *s;
      if (!node_insert(n, &root)) {
        printf("Find failed for %c and insert failed as well!\n", *s);
        abort();
      }
      if(!acmap_valid(pool, root, print_node)) {
        acmap_print(pool, root, print_node, 0);
        abort();
      }
      if (!node_find(*s, root)) {
...
*/
acmap_node_t *fill_data_structure_randomly(acpool_t *pool, const char *arg) {
  acmap_node_t *root = NULL;
  const char *p = arg;
  int len = strlen(p);
  int num_inserted = 0;
...
/*
	...
	the data structure.
*/
void find_everything(const char *arg, acmap_node_t *root) {
...
*/
void find_and_erase_everything(acpool_t *pool,
                               const char *arg,
                               acmap_node_t *root) {
  ...
  while (num_destroyed < len) {
    ...
    // acmap_print(pool, root, print_node, 0);
    if (node_to_erase) {
      if (!acmap_erase((acmap_node_t *)node_to_erase, &root)) {
...
*/
void print_using_reverse_iteration(acmap_node_t *root) {
  printf("print_using_reverse_iteration: ");
  acmap_node_t *n = acmap_last(root);
  while (n) {
    printf("%c", ((node_t *)n)->key);
    n = acmap_previous(n);
  }
...
*/
void destroy_using_iteration(acmap_node_t *root) {
  acmap_node_t *n = acmap_postorder_first(root);
  while (n) {
    acmap_node_t *next = acmap_postorder_next(n);
