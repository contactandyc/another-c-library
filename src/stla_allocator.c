...
static void _stla_dump_global_allocations(stla_allocator_t *a, FILE *out) {
  if (a->head) {
    fprintf(out,
            ...
            a->total_allocations * sizeof(stla_allocator_node_t));
...
void myCleanupFun(void) { stla_allocator_destroy(global_allocator); }
...
void *_stla_malloc_d(stla_allocator_t *a, const char *caller, size_t len,
  ...
  stla_allocator_node_t *n =
      (stla_allocator_node_t *)malloc(sizeof(stla_allocator_node_t) + len);
  if (!n) {
    if (a->thread_safe)
      pthread_mutex_lock(&a->mutex);
...
static stla_allocator_node_t *get_stla_node(stla_allocator_t *a,
...
void *_stla_realloc_d(stla_allocator_t *a, const char *caller, void *p,
  ...
  stla_allocator_node_t *n = get_stla_node(
      a, caller, p, "stla_realloc is invalid (p is not allocated?)");
...
void _stla_free_d(stla_allocator_t *a, const char *caller, void *p) {
  ...
  if (!a)
    a = global_allocator;
  stla_allocator_node_t *n =
      get_stla_node(a, caller, p, "stla_free is invalid (double free?)");
  if (a->thread_safe)
    pthread_mutex_lock(&a->mutex);
  if (n->previous)
