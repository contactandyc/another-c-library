...
void md5_update (md5_t *mdContext,
                unsigned char *inBuf,
                unsigned int inLen) {
  uint32_t in[16];
  int mdi;
...
void md5_final (md5_t *mdContext)
{
  uint32_t in[16];
  int mdi;
  unsigned int i, ii;
...
uint64_t md5_hash(const void *d, size_t len) {
    md5_t m;
    md5_init(&m);
    md5_update(&m, (unsigned char *)d, len);
    md5_final(&m);
    return md5_take64(&m);
}

uint64_t md5_hash_str(const char *s) {
    return md5_hash(s, strlen(s));
}
