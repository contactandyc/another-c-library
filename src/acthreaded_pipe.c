...
typedef struct {
  void *object;
  void *arg;
  acthreaded_pipe_f cb;
} acthreaded_pipe_object_t;
...
struct acthreaded_pipe_s {
  ...
  acthreaded_pipe_close_f close_cb;
  void *close_arg;
  ...
  acthreaded_pipe_create_global_arg_f create_global_arg;
  acthreaded_pipe_destroy_global_arg_f destroy_global_arg;
  void *global_arg;

  acthreaded_pipe_create_thread_arg_f create_thread_arg;
  acthreaded_pipe_clear_thread_arg_f clear_thread_arg;
  acthreaded_pipe_destroy_thread_arg_f destroy_thread_arg;
};

void *update_task(void *arg) {
  acthreaded_pipe_t *h = (acthreaded_pipe_t *)arg;
  while (true) {
...
void *do_task(void *arg) {
  ...
  acthreaded_pipe_t *h = t->h;
  int n;
  acthreaded_pipe_object_t obj;
...
acthreaded_pipe_t *_acthreaded_pipe_init(int num_threads,
                                               const char *caller) {
  acthreaded_pipe_t *h = (acthreaded_pipe_t *)_acmalloc_d(
      NULL, caller,
      sizeof(acthreaded_pipe_t) + (sizeof(thread_data_t) * num_threads),
      false);
#else
acthreaded_pipe_t *_acthreaded_pipe_init(int num_threads) {
  acthreaded_pipe_t *h = (acthreaded_pipe_t *)acmalloc(
      sizeof(acthreaded_pipe_t) + (sizeof(thread_data_t) * num_threads));
#endif
