...
typedef struct {
  ...
  on_http_f on_chunk_encoding;
  on_http_f on_chunk;
  on_http_f on_chunks_complete;
  on_http_f on_url;
...
static inline void save_current_header(ac_llhttp_ext_t *lh) {
  ac_http_header_t *h = (ac_http_header_t *)ac_pool_alloc(
      lh->http.pool, sizeof(ac_http_header_t));
  h->key = lh->header_key;
...
static int on_headers_complete(llhttp_t *parser) {
  ...
  ac_http_header_t *head = lh->http.headers;
  while (head) {
  ...
  if (lh->http.chunked) {
    lh->on_chunk_encoding((ac_http_t *)lh);
...
static int on_chunk_header(llhttp_t *parser) {
  ...
  if (parser->content_length == 0) {
    lh->on_chunks_complete((ac_http_t *)lh);
    return 0;
  }
  return 0;
}
...
static void on_http(ac_http_t *h) {}

static ac_http_t *ac_http_init(size_t pool_size, bool client,
                               on_http_f _on_url) {
...
ac_http_t *ac_http_service_init(on_http_f on_url, size_t pool_size) {
  return ac_http_init(pool_size, false, on_url);
}

ac_http_t *ac_http_client_init(on_http_f on_body, size_t pool_size) {
  return ac_http_init(pool_size, true, on_body);
}
...
bool ac_http_data(ac_http_t *h, const void *d, size_t len) {
  ac_llhttp_ext_t *hp = (ac_llhttp_ext_t *)h;
  return llhttp_execute(&hp->parser, d, len) ? false : true;
  /*
  // Process data one byte at a time
  char *p = (char *)d;
  for (size_t i = 0; i < len; i++) {
    if (llhttp_execute(&hp->parser, p + i, 1))
      return false;
  }
  return true;
  */
