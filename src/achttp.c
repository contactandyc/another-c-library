...
struct achttp_s {
  achttp_group_t *group;
  achttp_t *next;
  acasync_buffer_t *async_buffer;
  acpool_t *pool;
  void *arg;
  char *uri;
  char *method;
  ...
  uint32_t state;
  int pool_member;
  acbuffer_t *chunk_body_cache;
  accgi_t *query_cgi;
  accgi_t *body_cgi;
};

struct achttp_group_s {
  achttp_t *parser_pool;
  uint32_t pool_size;
  pthread_mutex_t lock;
  achttp_f on_headers;
  achttp_data_f on_request_end;
  achttp_f on_parsing_error;
  achttp_data_f on_body_chunk;
};

achttp_group_t *achttp_group_init(achttp_f on_headers,
                                        achttp_data_f on_body_chunk,
                                        achttp_data_f on_request_end,
                                        achttp_f on_parsing_error) {
  achttp_group_t *g = accalloc(sizeof(*g));
  pthread_mutex_init(&g->lock, NULL);
  if (!(g->on_headers = on_headers) || !(g->on_request_end = on_request_end) ||
      !(g->on_parsing_error = on_parsing_error)) {
    fprintf(stderr, "achttp_group_init() -- handlers must be "
...
static int parse_request_and_headers(achttp_t *parser, char *headers,
  ...
  if ((headers = memmem(headers, headers_length, "\r\n", 2))) {
    ...
    for (; p; p = memmem(p + 2, (end_headers - p) - 2, "\r\n", 2),
      ...
      ;
    parser->headers =
        acpool_calloc(parser->pool, sizeof(char *) * parser->num_headers);
    ...
    while (p < end_headers) {
      ...
      parser->headers[parser->num_headers++] =
          acpool_strndup(parser->pool, p, ep - p);
      p = ep + 2;
    }
  } else {
    end_req_line = end_headers;
  }
  req_line = acpool_strndup(parser->pool, req_line, end_req_line - req_line);
  char *p = req_line;
...
static void on_data(acasync_buffer_t *br) {
  achttp_t *p = acasync_buffer_get_arg(br);
  char *data;
  size_t data_length;
  while (1) {
    data = acasync_buffer_data(br);
    data_length = acasync_buffer_data_length(br);
    if (p->state & http_state_reading_headers) {
      ...
      uint64_t content_length =
          acuint64_t(achttp_param(p, header, "Content-Length", NULL), 0);
      char const *encoding =
          achttp_param(p, header, "Transfer-Encoding", NULL);
      if (content_length) {
        ...
        if (!acasync_buffer_advance_bytes(p->async_buffer, content_length,
                                             on_data)) {
          return;
        }
        data = acasync_buffer_data(br);
        ...
        if (!acasync_buffer_advance_to_string(p->async_buffer, "\r\n",
                                                 on_data)) {
          return;
        }
        data = acasync_buffer_data(br);
    ...
    if (p->state & http_state_reading_chunk_size) {
      ...
      if (sscanf(buf, "%zu", &chunk_size) != 1) {
        ...
        if (chunk_size > 0) {
          ...
          if (!acasync_buffer_advance_bytes(p->async_buffer, chunk_size,
                                               on_data)) {
            return;
          }
          data = acasync_buffer_data(br);
    ...
    if (p->state & http_state_reading_chunk_data) {
      ...
      if (p->group->on_body_chunk) {
        ...
        if (!p->chunk_body_cache) {
          p->chunk_body_cache = acbuffer_init(1024 * 4);
        }
        acbuffer_append(p->chunk_body_cache, data, data_length);
      }
      p->state ^=
    ...
    while (p->state & http_state_reading_footers) {
      if (data_length > 0) {
        ...
        p->post_size = 0;
        if (p->chunk_body_cache) {
          p->post_data = acbuffer_data(p->chunk_body_cache);
          p->post_size = acbuffer_length(p->chunk_body_cache);
        }
...
void achttp_set_arg(achttp_t *p, void *arg) {
  if (p)
    p->arg = arg;
}

void *achttp_get_arg(achttp_t *p) { return (p) ? p->arg : NULL; }

achttp_t *achttp_init(achttp_group_t *g) {
  achttp_t *res = NULL;
  if (g) {
    int pool_member = 1;
    pthread_mutex_lock(&g->lock);
    ...
    pthread_mutex_unlock(&g->lock);
    if (!res) {
      res = acmalloc(sizeof(*res));
      res->group = g;
      res->pool_member = pool_member;
      res->pool = acpool_init(1024);
      res->async_buffer = acasync_buffer_init();
      acasync_buffer_set_arg(res->async_buffer, res);
    }
    res->state = http_state_reading_headers;
    gettimeofday(&res->session_start, NULL);
    acasync_buffer_advance_to_string(res->async_buffer, "\r\n\r\n", on_data);
  }
  return res;
}

void achttp_release(achttp_t *p) {
  if (p) {
    acbuffer_destroy(p->chunk_body_cache);
    p->chunk_body_cache = NULL;
    if (p->pool_member) {
      acasync_buffer_clear(p->async_buffer);
      acpool_clear(p->pool);
      ...
      acasync_buffer_destroy(p->async_buffer);
      acpool_destroy(p->pool);
      acfree(p);
    }
  }
}

void achttp_parse(achttp_t *p, char const *data, size_t data_length) {
  if (p->state & http_state_read_complete) {
    p->group->on_parsing_error(p);
    return;
  }
  acasync_buffer_parse(p->async_buffer, data, data_length);
}

char const *achttp_method(achttp_t *p) { return p ? p->method : NULL; }

char const *achttp_uri(achttp_t *p) { return p ? p->uri : NULL; }

static char *get_header_param(achttp_t *p, char const *field,
                              size_t field_length) {
  char *res = NULL;
  if (p && field_length) {
...
static accgi_t *get_query_cgi(achttp_t *p) {
  if (!p->query_cgi && p->uri) {
    p->query_cgi = accgi_init(p->pool, p->uri);
  }
  return p->query_cgi;
}

static accgi_t *get_body_cgi(achttp_t *p) {
  if (!p->body_cgi) {
    char const *content_type = achttp_param(p, header, "Content-Type", NULL);
    if (p->post_size && content_type &&
      ...
      p->body_cgi = accgi_init(
          p->pool, acpool_strndup(p->pool, p->post_data, p->post_size));
    }
  }
  return p->body_cgi;
}

char const *achttp_param(achttp_t *p, achttp_param_location_t loc,
  ...
  if (p && key && key[0]) {
    ...
    case header:
      ...
      break;
    case query:
      res = accgi_str(get_query_cgi(p), key, NULL);
      break;
    case body:
      res = accgi_str(get_body_cgi(p), key, NULL);
      break;
    }
  }
...
char **achttp_params(achttp_t *p, achttp_param_location_t loc,
                        char const *key) {
  ...
  if (p && key && key[0]) {
    ...
    case header:
      break;
    case query:
      res = accgi_strs(get_query_cgi(p), key);
      break;
    case body:
      res = accgi_strs(get_body_cgi(p), key);
      break;
    }
  }
