...
actimer_t *_actimer_init(int repeat, const char *caller) {
  actimer_t *t =
      (actimer_t *)_acmalloc_d(NULL, caller, sizeof(actimer_t), false);
#else
...
void actimer_destroy(actimer_t *t) { acfree(t); }

/* get the number of times a task is meant to repeat */
int actimer_get_repeat(actimer_t *t) { return t->repeat; }

/* set the number of times a task is meant to repeat */
void actimer_set_repeat(actimer_t *t, int repeat) { t->repeat = repeat; }
