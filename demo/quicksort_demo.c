...
acsort_print_m(quicksort_print, sort_t);
acsort_m(quicksort, sort_t, less);
acsort_test_m(quicksort_test, less, sort_t);
...
void run_test(const char *test_name, sort_t *base, size_t num_elements,
  ...
  // REPEAT_TEST=1;
  printf("%s\n", test_name);
  actimer_t *copy_timer = actimer_init(REPEAT_TEST);
  ...
  actimer_t *base_timer = actimer_init(REPEAT_TEST);
  actimer_subtract(base_timer, copy_timer);
  actimer_start(base_timer);
  for (int i = 0; i < REPEAT_TEST; i++) {
    memcpy(base + num_elements, base, num_elements * sizeof(sort_t));
    qsort(base + num_elements, num_elements, sizeof(sort_t), compare_size);
  }
  actimer_stop(base_timer);

  actimer_t *test_timer = actimer_init(REPEAT_TEST);
  ...
  if (strchr(type, 'P'))
    quicksort_print(__ACFILE_LINE__, base + num_elements, num_elements,
                    print_sort_t);
  quicksort_test(__ACFILE_LINE__, base + num_elements, num_elements,
                 print_sort_t);

  printf("system qsort: %0.3fns\n", actimer_ns(base_timer));
  printf("acsort: %0.3fns\n", actimer_ns(test_timer));

  actimer_destroy(base_timer);
