...
void sort_entries_by_user(const char *in_file, const char *out_file,
  ...
  char *tmp_filename = (char *)ac_malloc(strlen(out_file) + 10);
  sprintf(tmp_filename, "%s_ut", out_file);
  ac_out_t *out = open_output(tmp_filename, 30 * 1024 * 1024,
                              compare_entry_by_user_and_time, NULL, 0);
  ...
  ac_io_record_t *r;
  size_t num_r = 0;
  while ((r = ac_in_advance_group(in, &num_r, compare_entry_by_user, NULL)) !=
         NULL) {
  }
  ac_out_write_record(out, r->record, r->length);
  ac_out_destroy(out);
...
void sort_entries_by_item(const char *in_file, const char *out_file) {
  ac_in_options_t in_opts;
  ac_in_options_init(&in_opts);
  ac_in_options_format(&in_opts, ac_io_fixed(sizeof(entry_t)));
  ac_in_t *in = ac_in_init(in_file, &in_opts);

  ac_out_options_t out_opts;
  ac_out_options_init(&out_opts);
  ac_out_options_buffer_size(&out_opts, 30 * 1024 * 1024);
  ac_out_options_format(&out_opts, ac_io_fixed(sizeof(entry_t)));

  ac_out_ext_options_t out_ext_opts;
  ac_out_ext_options_init(&out_ext_opts);
  ac_out_ext_options_use_extra_thread(&out_ext_opts);
  ac_out_ext_options_compare(&out_ext_opts, compare_entry_by_item_and_user,
                             NULL);
  // ac_out_ext_options_partition(&out_ext_opts, split_by_item, NULL);
  // ac_out_ext_options_num_partitions(&out_ext_opts, num_out);
...
int main(int argc, char *argv[]) {
  ...
  sort_entries_by_user(in_file, out_by_user);
  return 0;
}
