...
acmap_find_m(name_find, char, name_t, compare_name_for_find);
acmap_least_m(name_least, char, name_t, compare_name_for_find);
acmap_greatest_m(name_greatest, char, name_t, compare_name_for_find);

static inline int compare_name_partial(const char *a, const name_t *b) {
  return strncmp(a, get_name(b), strlen(a));
}

acmap_lower_bound_m(name_lower_bound, char, name_t, compare_name_partial);
acmap_upper_bound_m(name_upper_bound, char, name_t, compare_name_partial);

static inline bool less_name(name_t **a, name_t **b) {
  return strcmp(get_name(*a), get_name(*b)) < 0 ? true : false;
}

acsort_m(name_sort, name_t *, less_name);
/*
You can alternatively use a macro for acsort_m if needed
#define less_name_defined(a, b) (strcmp(get_name(*a), get_name(*b)) < 0)

acsort_m(name_sort, name_t *, less_name_defined);
*/
...
acsort_m(name_reverse, name_t *, reverse_less_name);
...
acsearch_m(name_search, char, name_t *, compare_name_for_search);
acsearch_least_m(name_search_least, char, name_t *, compare_name_for_search);
acsearch_greatest_m(name_search_greatest, char, name_t *,
                       compare_name_for_search);
acsearch_lower_bound_m(name_search_lower_bound, char, name_t *,
                          compare_partial_name_for_search);
acsearch_upper_bound_m(name_search_upper_bound, char, name_t *,
                          compare_partial_name_for_search);
...
void print_context(name_t *n) {
  print_full_name(n);
  printf("next: ");
  name_t *n2 = (name_t *)acmap_next((acmap_t *)n);
  if (n2)
    print_full_name(n2);
  else
    printf("\n");
  printf("previous: ");
  n2 = (name_t *)acmap_previous((acmap_t *)n);
  if (n2)
    print_full_name(n2);
  else
    printf("\n");
...
int main(int argc, char *argv[]) {
  ...
  acbuffer_t *bh = acbuffer_init(1024);
  acmap_t *root = NULL;
  ...
  while (fgets(str, 999, in) != NULL) {
    ...
    acbuffer_append(bh, &name, sizeof(name));
  }
  fclose(in);

  name_t **names = (name_t **)acbuffer_data(bh);
  size_t num_names = acbuffer_length(bh) / sizeof(name_t *);

  int repeat = 1000;
  name_t **names_backup =
      (name_t **)acpool_dup(pool, names, num_names * sizeof(name_t *));
  actimer_t *c = actimer_init(repeat);
  actimer_start(c);
  ...
  actimer_t *t = actimer_init(repeat);
  actimer_subtract(t, c);
  actimer_start(t);
  for (int i = 0; i < repeat; i++) {
    memcpy(names, names_backup, num_names * sizeof(name_t *));
  ...
  for (int i = 1; i < argc; i++) {
    printf("acmap_find_m: %s\n", argv[i]);
    name_t *n = name_find(argv[i], root);
    if (n)
      print_context(n);

    printf("acmap_least_m: %s\n", argv[i]);
    n = name_least(argv[i], root);
    if (n)
      print_context(n);

    printf("acmap_greatest_m: %s\n", argv[i]);
    n = name_greatest(argv[i], root);
    if (n)
      print_context(n);

    printf("acmap_lower_bound_m: %s\n", argv[i]);
    n = name_lower_bound(argv[i], root);
    if (n)
      print_context(n);

    printf("acmap_upper_bound_m: %s\n", argv[i]);
