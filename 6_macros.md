...
In accommon.h, I also defined ACFILE_LINE_MACRO, which is a macro meant for objects such as the actimer object.  This will become more evident as we work through the allocator object.  As usual, it's a good idea to define macros using ac or AC as a prefix to ensure that your code doesn't conflict with other codebases.  AC is an acronym for another c library.
...
*/
#define acparent_object(addr, base_type, field) (base_type *)((char *)addr-offsetof(base_type,field))
...
The acparent_object macro is useful for finding the address of a structure when given a pointer to a member of the structure.
...
int main( int argc, char *argv[] ) {
  ...
  point.x = 1;
  point.y = 2;
  point_t *p = acparent_object(&point.y, point_t, y);
