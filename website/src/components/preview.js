...
function Preview() {
  // Text content from cnt array
  const txt = cnt.map((val, i) => {
    return (
      <div key={i} className="Preview-cnt">
        <h2 className="Preview-h2">{val.ttl}</h2>
        <h3 className="Preview-h3">{val.sub}</h3>
        <p className="Prewiew-p">{val.dsc}</p>
        <Link to={val.pth}>
          <button className="Preview-btn">{val.btn}</button>
        </Link>
      </div>
    );
  });
